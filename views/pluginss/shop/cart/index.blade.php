...
@section('content')
    <h1 class="text-center line-1 anim-typewriter">{{ trans('shop::messages.cart.title') }}</h1>
    <div class="card card-body">
        @if(! $cart->isEmpty())
            <form action="{{ route('shop.cart.update') }}" method="POST">
                @csrf

                <table class="table cart-items w-100">
                    <thead>
                    <tr>
                        <th class="firstLetter" scope="col">{{ trans('messages.fields.name') }}</th>
                        <th class="firstLetter" scope="col">{{ trans('shop::messages.fields.price') }}</th>
                        <th class="firstLetter" scope="col">{{ trans('shop::messages.fields.total') }}</th>
                        <th class="firstLetter" scope="col">{{ trans('shop::messages.fields.quantity') }}</th>
                        <th class="firstLetter" scope="col">{{ trans('messages.fields.action') }}</th>
                    </tr>
                    </thead>
                    <tbody>
            ...
            <form method="POST" action="{{ route('shop.cart.clear') }}"
                  class="text-end mb-4 d-flex justify-content-start gap-2 p-2">
                @csrf
                <p class="text-end mb-1">
                    <button type="submit" class="btn btn-primary btn-sm cursorAnim">
                        <i class="bi bi-check-lg"></i> {{ trans('messages.actions.update') }}
                    </button>
                </p>
                <button type="submit" class="btn btn-danger btn-sm cursorAnim">
